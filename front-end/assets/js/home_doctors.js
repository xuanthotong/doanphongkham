...
async function showDoctorDetails(id) {
    ...
    if (reviewsContainer) {
        ...
        try {
            const res = await fetch(`${window.API_BASE}/api/doctors/${id}/reviews`);
...
function bookDoctor(id, event) {
    ...
    if (!isLoggedIn) {
        ...
        if (window.location.pathname.includes('patient.html')) {
            ...
            // 2. Tìm và tự động click vào thẻ Bác sĩ tương ứng (layout mới)
            setTimeout(() => {
                const toggleBtn = document.getElementById(`bdc-toggle-${id}`);
                if (toggleBtn) {
                    toggleBtn.click();
