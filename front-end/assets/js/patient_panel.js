...
function renderCommunityQA() {
    try {
        ...
        let filteredQuestions = window.allCommunityQA.filter(q => {
            ...
            const matchStatus = (window.currentQAStatus === 'all' || !window.currentQAStatus) ||
        ...
        window.currentFilteredQA = filteredQuestions;

        // Gom nhóm và lọc câu hỏi ẩn danh
        const uniqueUsers = new Set();
        let groupedQuestions = [];
        
        // Lọc trên danh sách đã filter theo keyword, status...
        filteredQuestions.forEach(q => {
            let isAnDanh = false;
            if (q.tieu_de && q.tieu_de.startsWith('[Ẩn danh]')) isAnDanh = true;
            else if (!q.nguoi_hoi || q.nguoi_hoi.trim() === '') isAnDanh = true;

            if (isAnDanh && q.benh_nhan_id != currentUserId) { // Dùng != để so sánh giá trị, không so sánh kiểu
                return;
            }
        ...
        filteredQuestions = groupedQuestions;
        ...
        // Lấy lại thông tin user ID để tô màu
        const currentUserInfoForRender = JSON.parse(localStorage.getItem('userInfo') || '{}');
        const currentUserIdForRender = currentUserInfoForRender ? currentUserInfoForRender.id : null;
        let qaHTML = '';
        paginatedQuestions.forEach(q => {
            const date = new Date(q.ngay_tao || Date.now());
            const timeString = `${String(date.getHours()).padStart(2, '0')}:${String(date.getMinutes()).padStart(2, '0')}`;
            const dateString = `${timeString} - ${String(date.getDate()).padStart(2, '0')}/${String(date.getMonth() + 1).padStart(2, '0')}/${date.getFullYear()}`;
            ...
            const isMyQuestion = (q.benh_nhan_id && q.benh_nhan_id == currentUserIdForRender);
            ...
            const tenNguoiHoiEncoded = encodeURIComponent(tenNguoiHoi);
            const isAnDanhStr = isAnDanh ? 'true' : 'false';
            const popupArgs = `decodeURIComponent('${identifierEncoded}'), decodeURIComponent('${tenNguoiHoiEncoded}'), '${isAnDanhStr}'`;
                    ...
                    <div class="qa-card-header" style="display:flex; justify-content:space-between; align-items:center;">
                        <div class="qa-user-info" style="display:flex; align-items:center; gap: 10px;">
                            ...
                            <div class="qa-meta" style="font-size: 13px; color: #64748B;">
                                <span class="qa-name" style="font-weight:bold; color:${nameColor};">${tenNguoiHoi}</span> • 
...
// ==================================================
function openQADetailPopup(identifier, tenNguoiHoi, isAnDanhStr, ignoreFilters = false) {
    const userInfo = JSON.parse(localStorage.getItem('userInfo') || '{}');
    const currentUserId = userInfo ? userInfo.id : null;

    const baseQA = (ignoreFilters || !window.currentFilteredQA) ? window.allCommunityQA : window.currentFilteredQA;

    const userAllQuestions = baseQA.filter(x => {
        let xIsAnDanh = false;
    ...
    if (isAnDanhStr === 'true' && userAllQuestions.length > 1) {
...
function filterQAByStatus(status) {
    window.currentQAStatus = status || 'all';
    window.currentPage = 1;
    renderCommunityQA();
}
    ...
    if (typeof window.allCommunityQA !== 'undefined' && window.allCommunityQA.length > 0) {
        ...
        if (matchedQA.length > 0) {
            ...
            matchedQA.forEach(q => {
                ...
                const tenNguoiHoiEncoded = encodeURIComponent(tenNguoiHoi);
                const isAnDanhStr = isAnDanh ? 'true' : 'false';
                const popupArgs = `decodeURIComponent('${identifierEncoded}'), decodeURIComponent('${tenNguoiHoiEncoded}'), '${isAnDanhStr}', true`;
